...
2. **Verify pre-conditions**
   ...
   **Handle states:**
   - If `state: "blocked"` (missing artifacts): show message, suggest `/opsx:propose` or `/rg:plan`
   - If `state: "all_done"`: congratulate, suggest `/opsx:archive` or `/rg:archive`
   - Otherwise: proceed

   Check `tests/` for test files. If none → "Run `/opsx:propose` or `/rg:plan` first (which generates tests)."
