...
| `/rg:plan` | Reads `exploration.md` → `/opsx:propose` → `/plan-creator` (Essentials) → delegates test generation → pauses for approval |
...
| `/rg:fix` | `.claude/fixes/<timestamp>-<name>.md` | ✅ |
...
- `/rg:plan` requires `/rg:explore` first. It will not proceed without `exploration.md`.
